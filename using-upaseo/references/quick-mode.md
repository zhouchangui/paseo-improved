...
> **判定规则的权威定义在主文件** `using-upaseo/SKILL.md` §0.2（micro/quick/full 三档的完整条件与边界）。本文件不重复判定规则，只给流程图与差异表；若与主文件冲突以主文件为准。

判定流程概要（完整条件见主文件 §0.2）：

1. **先判微改快速通道**：若满足主文件 §0.2 微改条件，走下文"微改快速通道执行"。
2. **再判快速模式**：不满足微改但满足主文件 §0.2 快速模式条件，走下文"执行流程"。
3. **其余进入完整仪式模式**：不在本文件范围。
...
微改快速通道不创建最小主计划、不创建迭代设计文档、不启动 `upaseo-loop`，也不新增失败测试。一旦实际改动超出主文件 §0.2 的边界，必须升级到快速模式或完整仪式模式。
