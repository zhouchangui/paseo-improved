...
任何被 Orchestrator 派生出的子 Agent，**必须在执行任何开发动作之前先使用 `view_file` 读取迭代设计与任务文档**。这是最小必要上下文。
同时，若本次开发涉及核心数据结构、数据库表、API 接口、路由或包结构的修改，子 Agent **必须同时优先读取对应的核心资产文档（如 `data_models.md`、`apis.md`、`modules.md`）**，确保与既有项目的架构实现保持高度的一致性。
...
- **任务**：若本轮迭代涉及核心数据结构/表结构、核心 API 接口/内部服务、包模块职责/路由页面等历史开发资产的变更，必须使用代码替换工具，规范地将增量变更写入对应的 `.paseo/story/` 资产文档中。
