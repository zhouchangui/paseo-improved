...
grep -q "执行 \`/upaseo-compact\` 时" "$compact_skill" 2>/dev/null && grep -q "先自动补齐再继续 compact" "$compact_skill" 2>/dev/null && pass "upaseo-compact 会先检查并自愈 hooks 安装" || fail "upaseo-compact 缺失 hooks 自愈安装规程"
