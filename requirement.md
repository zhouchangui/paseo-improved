...
- 每个迭代必须有**独立的设计与任务文档** (`iter_N_design_tasks.md`)。因为在极简工作流中，**设计与开发任务是合二为一、融为一体的**。该文档必须包含：
  ...
  - 用户可执行的手动验证步骤
- 每个迭代设计文档在实现前必须追加 **Design Council Log**：
  - `architecture-designer` 审查架构边界、模块归属、依赖方向和是否违反架构约束。
  - `feature-designer` 审查用户故事、功能边界、业务行为和与既有资产的一致性。
  - `test-strategist` 审查验收标准、日志/测试/browser/manual 证据链和失败条件。
  - 至少进行 1 轮反馈；若存在阻塞问题、验收不可执行或角色意见冲突，必须修正草案并进行第 2 轮复审。
  - 若第 2 轮仍无法收敛，暂停实现并向用户提交争议点。
- 每个迭代除了 Agent 自行验证以外，**必须给用户提供明确的验证内容和方法**。
- 快速模式也必须创建最小主计划 `.paseo/plans/<slug>.md`，并通过轻量 `upaseo-loop` 实现，不允许直接绕过 loop 修改。
...
1. **发布前置校验**：
   ...
   - 终极构建阻断：优先读取 `.paseo/story/coding_standards.md` 中记录的构建、Lint、类型检查和测试命令；若资产中没有定义，再执行项目惯例命令（如 `npm run build` 和 `npm run test`）。如果出现编译报错或测试失败，立即阻断发布。
2. **开发资产版本固化与 CHANGELOG 追加**：
   ...
   - 若产生 `.paseo/story/*` 或 `CHANGELOG.md` 修改，必须创建独立 release metadata commit，保证发布资产变更可追溯。
3. **物理环境与分支安全清理**：
